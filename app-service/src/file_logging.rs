...
impl FileLoggingManager {
    ...
    pub fn initialize_writer(&mut self) -> Result<Box<dyn Write + Send>> {
        ...
        let file_appender: Box<dyn Write + Send> = match self._config.rotation_policy.as_str() {
            "daily" => {
                let appender = rolling::daily(&self._config.log_dir, &self._config.file_prefix);
                Box::new(BufferedAppender::new(appender, self._config.buffer_size)?)
            }
            "hourly" => {
                let appender = rolling::hourly(&self._config.log_dir, &self._config.file_prefix);
                Box::new(BufferedAppender::new(appender, self._config.buffer_size)?)
            }
            "size_based" => {
                let appender = SizeBasedAppender::new(
                    ...
                    self._config.max_file_size_mb * 1024 * 1024, // Convert MB to bytes
                    self._config.max_files,
                )?;
                Box::new(BufferedAppender::new(appender, self._config.buffer_size)?)
            }
            _ => return Err(eyre!("Unsupported rotation policy: {}", self._config.rotation_policy)),
        };
    ...
    fn start_sync_task(&self, _writer: non_blocking::NonBlocking) {
        let sync_interval = self._config.sync_interval_secs;
