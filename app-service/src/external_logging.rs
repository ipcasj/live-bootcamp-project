...
impl ExternalLoggingClient {
    ...
    async fn flush_logs(
        ...
        let payload = match config.service_type.as_str() {
            ...
            "loki" => Self::prepare_loki_payload(logs)?,
    ...
    fn prepare_loki_payload(logs: &[LogEntry]) -> Result<String> {
        ...
        // Use current timestamp in nanoseconds for all logs to avoid timestamp parsing issues
        let timestamp_nanos = chrono::Utc::now().timestamp_nanos_opt()
            .unwrap_or_default()
            .to_string();
        
        let streams: Vec<serde_json::Value> = logs.iter().map(|log| {
            serde_json::json!({
                "stream": {
                    "job": log.service,
                ...
                "values": [[timestamp_nanos, log.message]]
            })
        }).collect();
        ...
        tracing::error!("DEBUG: Loki payload with {} logs: {}", logs.len(), payload_str);
