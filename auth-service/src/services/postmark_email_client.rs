...
use std::time::Duration; // For timeout handling
...
impl PostmarkEmailClient {
    // Constructor for creating a new PostmarkEmailClient instance with configuration
    pub async fn new(
        base_url: String,
        sender: String,
        ...
        let sender_email = Email::parse_from_str(&sender)?;
        let auth_token = Secret::new(authorization_token);
    ...
    // Constructor for creating a new PostmarkEmailClient instance (for tests)
...
impl EmailClient for PostmarkEmailClient {
    #[tracing::instrument(name = "Sending 2FA code email", skip(self, code))]
    async fn send_2fa_code(&self, email: &str, code: &str) -> Result<()> {
        let subject = "Your 2FA Code";
        let body = format!("Your 2FA verification code is: {}", code);
        
        let email_domain = Email::parse_from_str(email)?;
        self.send_email(&email_domain, subject, &body).await
    }
...
mod tests {
    ...
    fn email() -> Email {
        Email::parse_from_str(&SafeEmail().fake::<String>()).unwrap()
    }

    // Helper function to create a test email client
    fn email_client(base_url: String) -> PostmarkEmailClient {
        let http_client = Client::builder()
            .timeout(Duration::from_secs(5)) // 5 second timeout for tests
            .build()
            .unwrap();
        PostmarkEmailClient::new_with_client(base_url, email(), Secret::new(Faker.fake()), http_client)
    }
